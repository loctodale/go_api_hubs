...
func (s *grpcServer) CheckApisKeyIsExisted(ctx context.Context, req *pb.CheckApisKeyIsExistedRequest) (*pb.CheckApisKeyIsExistedResponse, error) {
	result, err := s.apisKeyService.CheckKeyIsExisted(req)
	if err != nil {
		fmt.Println("Error::: ", err)
	}
	return result, nil
