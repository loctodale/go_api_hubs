...
type ApisKeyRepository interface {
	...
	GetApisKeyById(id pgtype.UUID) (database.TblApisKey, error)
}
...
func (r *apisKeyRepository) GetApisKeyById(id pgtype.UUID) (database.TblApisKey, error) {
	result, err := r.query.GetApisKeyById(global.Ctx, id)
	if err != nil {
		return database.TblApisKey{}, err
	}
	return result, nil
