...
type ApisKeyService interface {
	...
	CheckKeyIsExisted(req *pb.CheckApisKeyIsExistedRequest) (*pb.CheckApisKeyIsExistedResponse, error)
}
...
func (a apisKeyService) CheckKeyIsExisted(req *pb.CheckApisKeyIsExistedRequest) (*pb.CheckApisKeyIsExistedResponse, error) {
	var uuid pgtype.UUID
	err := uuid.Scan(req.Id)
	if err != nil {
		return &pb.CheckApisKeyIsExistedResponse{
			BaseResponse: &pb.BaseResponse{
				Code:    500,
				Message: err.Error(),
			},
	...
	result, err := a.apisKeyRepository.GetApisKeyById(uuid)
	if err != nil {
		return &pb.CheckApisKeyIsExistedResponse{
			BaseResponse: &pb.BaseResponse{
				Code:    500,
