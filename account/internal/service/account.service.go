...
type Service interface {
	...
	VerifyProviderAccount(account string, otp string) (pb.BaseResponseMessage, error)
}
...
func (a *accountService) VerifyAccount(account string, otp string) (pb.BaseResponseMessage, error) {
	...
	// Create wallet
	result, err := a.repository.GetLoginAccount(account)
	if err != nil {
		return pb.BaseResponseMessage{Code: 400, Message: err.Error()}, err
	}
	conn, err := grpc.Dial("payment-service:8080", grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		return pb.BaseResponseMessage{Code: 400, Message: err.Error()}, err
	}
	defer conn.Close()
	paymentClient := paymentPb.NewPaymentServiceClient(conn)
	_, err = paymentClient.CreateNewWallet(global.Ctx, &paymentPb.CreateWalletRequest{
		Method:      "TEST",
		Amount:      10,
		Type:        1,
		ReferenceId: "TEST",
...
func (a *accountService) VerifyProviderAccount(account string, otp string) (pb.BaseResponseMessage, error) {
	hashKey := a.utils.GetHash(strings.ToLower(account))
	userKey := fmt.Sprintf("u:%s:otp", hashKey)
	foundOTP, err := global.Rdb.Get(global.Ctx, userKey).Result()
	if err != nil {
	...
	// Create ACL Group for Consumer (Kong)
	err = a.utils.AddConsumerToACLGroup(account, "provider")
	if err != nil {
		return pb.BaseResponseMessage{Code: 400, Message: err.Error()}, err
	}
